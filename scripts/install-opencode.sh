...
#   plugin  → .opencode/plugins/   (the thrash nudge)
...
# 3. Plugin — the thrash nudge.
mkdir -p "$PLUGINS_DIR"
cp "$SRC/.opencode/plugins/pipeline.js" "$PLUGINS_DIR/"
cp "$SRC/hooks/thrash-detector.mjs" "$PLUGINS_DIR/"
...
Done. Open the target project in opencode and restart it.
  ...
  • the thrash nudge loads from the plugin automatically
