...
<!--
  ...
  - opencode port: the neutral `model: fast` is dropped so the builder inherits
    the session model — set `model: <provider>/<id>` here to pin a faster one.
  - All tools allowed (source set: read, write, edit, bash, grep, glob), so no
    `tools` restriction. The builder is the EXECUTOR: keep the
    producer/evaluator/executor boundary — it must not redesign (planner's job)
    or self-approve (reviewer's job).
-->
