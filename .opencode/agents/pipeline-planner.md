...
Decide variant count from product maturity + change breadth (infer from the WP and design system). Prefer 1 in a mature app with many screens. Use 3 only for early+broad product UI, or for a new **base** design-system primitive. Let the pipeline-reviewer score when 2+.
