...
4. **Take the shortest working path.** Stop at the first option that works: delete or reuse existing code → standard-library or native feature → already-installed dependency → minimum new code. Prefer one changed file to several. Do not add a helper, abstraction, dependency, configuration switch, compatibility layer, or scaffolding unless the current acceptance criteria require it. When two solutions work, choose the smaller diff; when unsure, under-engineer.
5. **Follow existing patterns.** Read existing code in the same area before writing new code. Match the conventions you find.
6. **Keep WP IDs in `.pipeline/**`.** Outside it, use domain names — no exact or derived ID in paths, content, identifiers, test artifacts, UI, or VCS/PR metadata. Leaking plan → BLOCKER. Before completion, search; zero leaks.
