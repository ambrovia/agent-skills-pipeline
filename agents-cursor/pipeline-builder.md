...
- Mechanical check results injected with the brief are evidence; do not re-run them unless you dispute
  them.
...
failures caused by the change from pre-existing failures; fix only the former unless
