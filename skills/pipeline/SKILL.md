...
**Ordering principle:** lock the **requirement** with a human before agents explore design or
architecture. Lock **design** with a human (when UI) before architecture plans implementation.
Architecture must **prove feasibility** (web research + mini POCs) before the critique loop clears
the plan for build.

| Phase | Persona | Model | Skills | Purpose |
|---|---|---|---|---|
| 1a | **pipeline-planner** | `{{models.design}}` | `refine` (if needed) | Write `requirements.md` — sharpen the goal, success, scope, guide draft; update `plan.md` only if scope/ACs shift. Runs when the goal is unclear or the WP introduces/reshapes a noun; otherwise confirm the seeded plan is sharp enough for human review. |
| 2a | **pipeline-reviewer** + **pipeline-planner** | `{{models.review}}` | `refine-critique` (if `refine` ran) → pipeline-planner revision loop | Score `requirements.md` before any human or design work — judge whether refine sharpened in the right direction against `plan.md`. CRITICAL/WARNING → planner revises `requirements.md` (**max 3 rounds**). |
| **2.5a** | **pipeline-planner** (or orchestrator park) | — | `human-concept-review` **pass: requirements** | **Mandatory — never skipped.** Founder reviews and approves `requirements.md` (value, success, scope, guide draft, AC alignment) against the `plan.md` spec. Interactive + founder present → planner revises to approval, sets `approvals.requirements` in `progress.json`. Autonomous / no founder → **park** (`status: awaiting-human-concept-review`). No design or architecture may start until this pass completes. |
| 1b | **pipeline-planner** | `{{models.design}}` | `design` | Variant exploration when the WP has a UI surface. Consumes the **approved** `requirements.md`. **Skip if no UI / no design system.** |
| **2.5b** | **pipeline-planner** (or orchestrator park) | — | `human-concept-review` **pass: design** | **Mandatory when UI ran.** Founder reviews the rendered variant in the component viewer + refreshed guide draft. Launches viewer (idempotent). Autonomous → park. **Skip only when Phase 1b design was skipped.** |
| 1c | **pipeline-planner** | `{{models.design}}` | `architecture` | Write `architecture.md` + `feasibility.md` — technical plan + **feasibility probes** (web research, mini POCs) proving load-bearing assumptions. Consumes `plan.md` + approved `requirements.md` (+ approved design when UI); update `plan.md` only if the plan changes. |
| 2b | **pipeline-reviewer** + **pipeline-planner** | `{{models.review}}` | `design-critique` (if design ran) → `architecture-critique` → pipeline-planner revision loop | Independent evaluation of design + `architecture.md`. CRITICAL/WARNING → planner revises (**max 3 rounds**). Builder receives a clean, approved `architecture.md` (+ `plan.md` spec) in `.pipeline/work/<id>/`. |
...
- **Refine critique loop (Phase 2a):** if findings are CRITICAL/WARNING, send them to the pipeline-planner,
  who revises `requirements.md` and the pipeline-reviewer re-critiques. Repeat until score ≥ 7 or **3 rounds**
  are reached. If it never clears: mark `blocked` with reason `concept-or-spec-misalignment`.
- **Human concept review:** never skip, never auto-approve. Autonomous runs **park** — they do not proceed
  to design, architecture, or build without founder approval. On resume, the founder runs
  `/human-concept-review <id>` (or the orchestrator continues from the parked `currentStep`).
- **Critique loop (Phase 2b):** if findings are CRITICAL/WARNING, send them to the pipeline-planner,
