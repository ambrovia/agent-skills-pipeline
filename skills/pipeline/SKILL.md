...
The pipeline-planner writes the approved plan for each WP to `.pipeline/plans/<id>.md` (concept + design
spec + architecture + acceptance criteria, as left after the Phase 2 critique loop). It is the
durable producer→consumer handoff: the pipeline-builder and pipeline-reviewer read it instead of relying on a warm
producer session.
...
|---|---|---|---|---|
| 1 | **pipeline-planner** | `{{models.design}}` | `refine` (if needed) → `design` → `architecture` | Requirement (value + noun shape + guide draft) + variant exploration (if UI) + technical plan. Production only. `refine` runs only when the WP's goal is unclear or it introduces/reshapes a noun; skip when the requirement is already sharp in `{{paths.docs}}`. |
| 2 | **pipeline-reviewer** + **pipeline-planner** | `{{models.review}}` | `refine-critique` (if `refine` ran) → `design-critique` → `architecture-critique` → pipeline-planner revision loop | Independent evaluation. CRITICAL/WARNING findings → pipeline-planner revises, pipeline-reviewer re-critiques (**max 3 rounds**). Builder receives a clean, approved plan. |
