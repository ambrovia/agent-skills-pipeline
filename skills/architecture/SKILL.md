...
- **Engineering tier** (`prototype | mvp | production | critical`) from `plan.md` — calibrate to the customer: `prototype` is a manual/internal demonstration with key features allowed to be absent; `mvp` gives tolerant early users a mostly working core while auxiliary features may be absent; `production` is ordinary software for ordinary users and should simply work normally; `critical` is for enterprise, regulated, contractual, or genuinely high-consequence customers. Compliance evidence, audit trails, formal rollback machinery, elaborate observability, exhaustive failure handling, and similar controls belong only in `critical` unless a concrete acceptance criterion or demonstrated risk requires them. Planning above or below the tier is a defect. Trust it as set; do not re-question it.
...
6. **Security & abuse cases** — list only plausible risks introduced or changed by this work and the proportionate response. Examples include an actual untrusted-input boundary, a new credential surface, user-controlled paths, or user-controlled outbound URLs. Do not require rate limits on every write or a ceremonial N/A checklist. Enterprise threat-model evidence belongs to `critical` unless explicitly required.
