...
3. **Inline annotations** — search the changed files for current `@lore` annotations (one active
   annotation per repository-global `@lore-id`; source history is not lore) and for
