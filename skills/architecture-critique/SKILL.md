...
Validate `technicalTaskDag`, then judge the split itself. Default to one leaf; require each split to reduce context or expose a dependency. Check that ownership covers all writes, context is a useful starting set, and parallel leaves share no mutable state or unsettled contract. Require an integration leaf for cross-leaf behavior. Mechanical leaves must be fully specified and judgment-free.
