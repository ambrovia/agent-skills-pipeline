...
Judge the technical task split. Default to one leaf; require each split to reduce context or expose a dependency. Check that ownership covers all writes, context is a useful starting set, and parallel leaves share no mutable state or unsettled contract. Require an integration leaf for cross-leaf behavior. Mechanical leaves must be fully specified and judgment-free.
...
### 9. Tier-fit — default to under-engineering

The simplest solution is the default and the fewest changed files win. Score against the WP's Engineering tier (`plan.md`: `prototype | mvp | production | critical`), but require only guarantees tied to the current customer and acceptance criteria. Use this classification:
...
- **Correctly under-engineered:** the smallest design that lets the tier's current customer complete every acceptance criterion, preserves baseline security, and prevents likely data loss. Prefer reuse, standard-library/native features, installed dependencies, and minimum new code—in that order.
- **Over-engineered:** any file, layer, abstraction, option, dependency, or control not demanded by a current acceptance criterion, existing convention, likely customer failure, or demonstrated risk. Examples: feature flags, abstract bases, plugin points, registries, speculative extensibility, duplicate validation, back-compat shims with no callers, retries, elaborate observability, formal rollback machinery, and defensive abstractions.
- **Actually under-built:** the promised task cannot work for the tier's current customer, or the plan omits baseline security, likely data-loss prevention, or a guarantee explicitly required for that tier/customer. Do not use "under-engineered" for missing hypothetical scale, flexibility, reuse, or enterprise controls.
- **Tie-breaker:** if reasonable reviewers could classify an item either way, call it over-engineering and remove it. Add complexity later only when a concrete requirement or measured failure appears.
