...
| "Dependency on X is just a logical ordering" | Then it's not a dependency. The scheduler will serialize via real `done` blockers. Logical ordering is what the pipeline-planner uses to plan tasks within a work package. |
...
| "This is a small fix, it doesn't need a work package" | Then it's a commit on an open work package or a one-off PR — not a thing the pipeline picks up. The per-track coordination file (`.pipeline/<track>.md`) is the queue; if the WP isn't registered there with its own `.pipeline/work/<id>/` folder, the pipeline won't build it. |
...
| "This AC is too vague for the pipeline-builder" | The pipeline-builder doesn't read ACs directly — they read the plan from `/architecture`, which translates each AC into specific tasks, types, and verification methods. ACs are the contract with the pipeline-reviewer, not the pipeline-builder's instruction manual. |
