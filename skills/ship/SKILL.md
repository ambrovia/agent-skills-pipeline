...
Confirm the intended diff and, when a work package exists, that no WP identifier leaks outside
`.pipeline/**`. Stop rather than repairing product work in ship.
...
1. When a work package ran, consolidate `.pipeline/work/<id>/`. Fold the approved requirement, the
   ...
   Consolidation is editorial: never change the work-package outcome, constraints, or acceptance criteria,
   and never drop a decision a future reader needs. Anything worth keeping is in `plan.md` before the rest
   is removed — including every deferral, known limitation, and accepted residual risk that outlives the
   run. A deferral recorded only in a deleted artifact is a deferral nobody will ever act on.
