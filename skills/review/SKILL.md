...
Audit changed paths/content and VCS/PR metadata for the WP ID. Exact or derived
matches stay in `.pipeline/**`; any leak is CRITICAL, verdict NOT DONE.
...
3. **Security** — review plausible risks created or changed by the work, calibrated to its customer and trust boundaries. Check relevant authorization, validation, injection/XSS, workspace escape, credential, and concurrency risks; do not mechanically apply every threat framework to every change. Search beyond changed files only as far as needed to verify affected call paths. Rate limiting, exhaustive threat modeling, and other enterprise controls are findings only when an acceptance criterion, demonstrated abuse risk, project rule, or `critical` customer context requires them.
