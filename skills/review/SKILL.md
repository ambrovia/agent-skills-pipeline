...
4. **Adversarial (hostile personas)** — stop checking contracts; try to break the code. Read the diff as **The Saboteur** (could a silent regression or scale-only race pass tests and review?), **The New Hire** (what takes 10 minutes to understand that should take 10 seconds? which name lies?), and **The Auditor** (does every claim in the change description match the diff — is the thing it says it wired up actually wired up?). Also: over-abstraction, missing error paths, type-safety escapes (count every one), coupling, weak tests, performance traps, mocked system-under-test (an integration proven only by a stub/mock of the very system it integrates is a false green — an absent binary/credential must surface as unproven, not skip to green), and the sanity-break check (a fix or enforcement test must go red when the change is reverted).
