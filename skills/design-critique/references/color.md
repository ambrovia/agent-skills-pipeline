...
> Cross-check every concrete value here against the project's own token reference and `{{designSystem.tokens}}` — those override any example below. The hardcoded-color and raw-color-utility checks are also enforced by the project's verify command, {{verify}}; this reference is the judgment layer. Status-color philosophy (full-monochrome vs. semantic accents) varies by project — defer to the design system's stated philosophy.
...
The exact token names and values come from `{{designSystem.tokens}}`. The structure below is the common shape; map it to the project's names.

### Surfaces (elevation through lightness)

Closer to the user = lighter. This simulates overhead lighting.
...
**Never skip levels.** A card surface should not sit on app chrome directly — it needs a content-area surface behind it.

### Text (three levels only)

| Level | Usage |
|-------|-------|
| high-emphasis | Primary content: titles, values, active nav. ~90% white in dark mode. |
| mid-emphasis | Secondary: body text, descriptions, inactive nav. ~67% white. |
| low-emphasis | Tertiary: timestamps, IDs, hints, mono labels. ~50% white. |
...
- Status colors appear as low-opacity backgrounds (commonly ~14% via `color-mix`).
...
- **No pure white (`#ffffff`) text.** Use off-white for high-emphasis text.
...
- Raw hex values (`#aabbcc`) — use a token reference instead.
- RGB/HSL values (`rgb(...)`, `hsl(...)`) — use tokens.
- Framework color utilities (`text-blue-500`, `bg-gray-800`) — use token-based values.
- Opacity on colored elements without using token-defined tint values.

**The only acceptable raw color values** are inside the token definitions themselves and in anti-pattern test fixtures.

## Common Mistakes

| Mistake | Fix |
|---------|-----|
| Using a raw gray utility for cards | Use the card surface token |
...
| Pure white (`#fff`) text | Use the off-white high-emphasis text token |
