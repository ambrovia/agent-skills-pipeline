...
6. **Default to under-engineering.** The simplest solution and the fewest changed files win. Stop at the first option that satisfies the current acceptance criteria: reuse existing code → standard-library or native feature → installed dependency → minimum new code. Add no abstraction, dependency, configuration, compatibility layer, or infrastructure for a hypothetical future. Under-engineering means the current customer can complete the promised task, with baseline security and no likely data loss; over-engineering means anything beyond that without a concrete requirement or demonstrated risk. If classification is uncertain, classify it as over-engineering and remove it. `/work-planning` owns the tier; higher tiers raise only explicitly required guarantees.
