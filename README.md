...
- **[`hooks/`](hooks/)** — a session-start hook and a conservative post-edit thrash detector. It nudges only on repeated equivalent actions/results (or cautiously when result data is absent), stores bounded hashes rather than source text, and serializes concurrent events for one session with a bounded 45 ms wait before failing open. Output is wired for Claude, Cursor, Gemini, Copilot, and opencode; Codex installs silent no-fail wrappers because Codex 0.142.5 rejects SessionStart/PostToolUse stdout for these lifecycle hooks. Cursor plugin hooks live in [`hooks/cursor-hooks.json`](hooks/cursor-hooks.json) and use `${CURSOR_PLUGIN_ROOT}` for install-path resolution.
